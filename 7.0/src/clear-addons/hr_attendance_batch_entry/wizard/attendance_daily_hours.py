...
class attendance_department(osv.osv_memory):
    ...
    def _get_pmstart(self):
        return ('13', '00')
    
    def _get_coldstore_name(self):
        return 'Coldstore'
    ...
    def _set_lines_from_records(self, sin, sout, vals):
        ...
        amhourstart = '07'
        amminstart = '00'
        pmhourstart, pmminstart = self._get_pmstart()
        dtCounter = False
        isColdroom = False
        for i in range(0, len(sin)):
            start = datetime.strptime(sin[i], '%Y-%m-%d %H:%M:%S')
            
            # Sometimes we have a sign-in without a corresponging sign-out. In
            # this case we simply put in a nonsense value.
            #
            if len(sout) < (i + 1):
                worked_hours = -24
            else:
            ...
            midday = datetime.strptime(start.strftime(self._get_midday_str()), '%Y-%m-%d %H:%M:%S')
    ...
    def _default_time_vals(self, cr, uid, eid, context=None):
        ...
        if e.department_id and e.department_id.name.find(self._get_coldstore_name()) != -1:
            res = {'amhour': '21',
                   'ammin': '00',
                   'pmhour': '02',
                   'pmmin': '00',
            ...
            pmhour, pmmin = self._get_pmstart()
            res = {'amhour': '07',
                   'ammin': '00',
                   'pmhour': pmhour,
                   'pmmin': pmmin
    ...
    def _get_lines(self, cr, uid, context=None):
        ...
        e_ids = weekly_obj.get_employees(cr, uid, weekly, context=context)
        ...
        for eid in e_ids:
            ...
            if eedata.get('contract_id', False):
                ...
                if len(sched_ids) > 0:
                    ...
                    for d in rest_days:
                        ...
                        if otHours > 0.01:
                            ...
                            if employee.department_id and employee.department_id.name.find(self._get_coldstore_name()) != -1:
                                times = ['21:00:00', '01:00:00', '02:00:00', '06:00:00']
                            else:
                                times = self._get_shift_times()
    ...
    def add_records(self, cr, uid, ids, context=None):
        ...
        for lid in lines:
            ...
            weekly_obj.write(cr, uid, weekly_id, {'init_time': datetime.now().strftime(OE_DATETIME_FORMAT)},
                             context=context)
