...
class policy_ot(osv.Model):
    ...
    def get_codes(self, cr, uid, idx, context=None):
        ...
        res = []
        [res.append((line.code, line.name, line.type, line.rate, line.accrual_policy_line_id.id, line.accrual_policy_line_id.code, line.accrual_rate, line.accrual_min, line.accrual_max)) for line in self.browse(cr, uid, idx, context=context).line_ids]
        return res
    
    def daily_codes(self, cr, uid, idx, context=None):
        
        res = []
        [res.append((line.code, line.name)) for line in self.browse(cr, uid, idx, context=context).line_ids if line.type == 'daily']
        return res
    
    def restday_codes(self, cr, uid, idx, context=None):
        
        res = []
        [res.append((line.code, line.name)) for line in self.browse(cr, uid, idx, context=context).line_ids if line.type == 'weekly' and line.weekly_working_days > 0]
        return res
    
    def restday2_codes(self, cr, uid, idx, context=None):
        
        res = []
        [res.append((line.code, line.name)) for line in self.browse(cr, uid, idx, context=context).line_ids if line.type == 'restday']
        return res
    ...
    def holiday_codes(self, cr, uid, idx, context=None):
        ...
        res = []
        [res.append((line.code, line.name)) for line in self.browse(cr, uid, idx, context=context).line_ids if line.type == 'holiday']
        return res
