...
class hr_infraction(osv.Model):
    ...
    _columns = {
        ...
        'action_ids': fields.one2many('hr.infraction.action', 'infraction_id', 'Actions',
                                      readonly=True),
        'manager_id': fields.many2one('hr.employee', 'Issued By', required=True),
        'department_id':fields.related('employee_id', 'department_id', string='Department',
    ...
    def onchange_employee(self, cr, uid, ids, employee_id, context=None):
        ...
        res = {'value': {'manager_id': False}}
        if employee_id:
            data = self.pool.get('hr.employee').read(cr, uid, employee_id, ['parent_id'],
                                                     context=context)
            if data.get('parent_id', False):
                res['value']['manager_id'] = data['parent_id'][0]
        return res
