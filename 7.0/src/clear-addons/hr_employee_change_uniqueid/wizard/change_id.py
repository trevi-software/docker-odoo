...
class change_id_wizard(orm.TransientModel):
    
    _name = 'log.change.employee.uniqueid.wizard'
    _description = 'Employee Unique ID Change Wizard'
    
    _columns = {
        'employee_id': fields.many2one('hr.employee', 'Employee', required=True),
        'prev_value': fields.char('Current Value', readonly=True),
        'new_value': fields.char('New Value', size=IDLEN, required=True),
    }
    ...
    def onchange_employee_id(self, cr, uid, ids, employee_id, context=None):
        ...
        if employee_id:
            ...
            res['value']['prev_value'] = ee.employee_no_rand
        return res
    
    def do_change(self, cr, uid, ids, context=None):
        
        wizard = self.browse(cr, uid, ids[0], context=context)
        change_id = self.pool.get('log.change.employee.uniqueid').create(cr, uid,
                                                       {'employee_id': wizard.employee_id.id,
                                                        'prev_value': str(wizard.employee_id.employee_no_rand)},
                                                       context=context)
        if not change_id:
            orm.except_orm(_('Error'), _('Unable to log ID change!'))
        
        self.pool.get('hr.employee').write(cr, uid, wizard.employee_id.id,
                                           {'employee_no_rand': wizard.new_value},
                                           context=context)
