...
class hr_job(osv.Model):
    ...
    def _get_all_child_ids(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for i in ids:
            result[i] = self.search(cr, uid, [('parent_id', 'child_of', i)], context=context)
    ...
    def _check_recursion(self, cr, uid, ids, context=None):
        ...
        level = 100
        while len(ids):
            cr.execute('select distinct parent_id from hr_job where id IN %s',(tuple(ids),))
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if job and job.parent_id:
            ...
            if parent_id and vals.get('employee_id'):
                ee_obj.write(cr, uid, vals['employee_id'], {'parent_id': parent_id},
                             context=context)
        ...
        if job:
            ...
            if len(job_child_ids) > 0:
                ee_ids = ee_obj.search(cr, uid, [('job_id', 'in', job_child_ids)])
                if len(ee_ids) > 0:
                    parent_id = False
                    for ee in job.employee_ids:
                        parent_id = ee.id
                    if parent_id:
                        ee_obj.write(cr, uid, ee_ids, {'parent_id': parent_id}, context=context)
            
        return res
    
    def write(self, cr, uid, ids, vals, context=None):
        
        res = super(hr_contract, self).write(cr, uid, ids, vals, context=context)
        ...
        if job and job.parent_id:
            ...
            if parent_id:
                for contract in self.browse(cr, uid, ids, context=context):
                    ee_obj.write(cr, uid, contract.employee_id.id, {'parent_id': parent_id}, context=context)
...
class hr_department(osv.Model):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if vals.has_key('manager_id'):
            datas = self.read(cr, uid, ids, ['manager_id'], context=context)
            for d in datas:
                if d['manager_id'] and d['manager_id'][0] not in manager_ids:
                    manager_ids.append(d['manager_id'][0])
        
        res = super(hr_department, self).write(cr, uid, ids, vals, context=context)
        
        ee_obj = self.pool.get('hr.employee')
        if vals.has_key('manager_id'):
            manager_id = vals['manager_id']
            manager_ids.append(vals['manager_id'])
            ee_ids = ee_obj.search(cr, uid, [('department_id', 'in', ids), ('active', '=', True),
                                             ('id', 'not in', manager_ids)], context=context)
            ee_obj.write(cr, uid, ee_ids, {'parent_id': manager_id}, context=context)
