...
class hr_schedule(osv.osv):
    ...
    _columns = {
        ...
        'template_id': fields.many2one('hr.schedule.template', 'Schedule Template'),
        'used_template_id': fields.many2one('hr.schedule.template', 'Schedule Template Used', readonly=True),
        'detail_ids': fields.one2many('hr.schedule.detail', 'schedule_id', 'Schedule Detail'),
        ...
        'validated': fields.boolean('Validated ?', readonly=True),
    }
        ...
        while d <= dTo:
            ...
            if len(ids) == 0:
                ...
                ee_data = self.pool.get('hr.employee').read(cr, uid, employee_id, ['name'],
                                                            context=context)
                # XXX - tripping up on this (29/12/2018). Find out why, but skip for now.
                d += relativedelta(days= +7)
                continue
                # XXX - End
    ...
    def add_restdays(self, cr, uid, sched_id, template, field_name, rest_days=None, context=None):
        
        restday_ids = []
        if rest_days == None:
            for rd in template.restday_ids:
                restday_ids.append(rd.id)
        else:
            restday_ids = self.pool.get('hr.schedule.weekday').search(cr, uid,
                                                                      [('sequence', 'in', rest_days)],
                                                                      context=context)
            # XXX - Work around a bug in the ORM that doesn't always find a record if
            #       the sequence is 0. Uggh!
            #
            if 0 in rest_days:
                restday_ids2 = self.pool.get('hr.schedule.weekday').search(cr, uid,
                                                                          [('sequence', '=', 0)],
                                                                          context=context)
                restday_ids += [r for r in restday_ids2 if r not in restday_ids]
        if len(restday_ids) > 0:
            self.write(cr, uid, sched_id, {field_name: [(6, 0, restday_ids)]}, context=context)
    ...
    def create_details(self, cr, uid, sched_id, template_id=None, context=None):
        ...
        if template_id == None:
            ...
            template = self.pool.get('hr.schedule.template').browse(cr, uid, template_id,
                                                                    context=context)
        ...
        if template:
            ...
            leaves = []
            local_tz = timezone(template.tz)
            _dtStart = local_tz.localize(datetime.strptime(schedule.date_start + ' 00:00:00', OE_DTFORMAT), is_dst=False)
            _dtEnd = local_tz.localize(datetime.strptime(schedule.date_end + ' 23:59:59', OE_DTFORMAT), is_dst=False)
            _start = _dtStart.astimezone(utc).strftime(OE_DTFORMAT)
            _end = _dtEnd.astimezone(utc).strftime(OE_DTFORMAT)
            ...
            while dCount <= dCountEnd:
                ...
                if dCount == dSchedStart:
                    self.add_restdays(cr, uid, schedule.id, template, 'restday_ids1', context=context)
                elif dCount == dSchedStart + relativedelta(days= +7):
                    self.add_restdays(cr, uid, schedule.id, template, 'restday_ids2', context=context)
                elif dCount == dSchedStart + relativedelta(days= +14):
                    self.add_restdays(cr, uid, schedule.id, template, 'restday_ids3', context=context)
                elif dCount == dSchedStart + relativedelta(days= +21):
                    self.add_restdays(cr, uid, schedule.id, template, 'restday_ids4', context=context)
                elif dCount == dSchedStart + relativedelta(days= +28):
                    self.add_restdays(cr, uid, schedule.id, template, 'restday_ids5', context=context)
                ...
                for worktime in template.worktime_ids:
                    ...
                    # We haven't yet made adjustments for shifts that go on
                    # into the next day. So, this date represents the correct
                    # date for all worktimes (whether they cross over into the
                    # next day or not).
                    dDay = locldtStart.date()
                    ...
                    # Hook to allow derived modules to say whether we should
                    # create this detail or not.
                    _hook_skip = self.skip_create_detail(
                        cr, uid, schedule, worktime.dayofweek, dDay,
                        utcdtStart, utcdtEnd, context=context)

                    if not _skip and not _hook_skip:
                        val = {
                            'name': schedule.name,
                        ...
                        self.write(cr, uid, sched_id, {'detail_ids': [(0, 0, val)],
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        # If the schedule template is a rolling rest day then find out what last
        # week's schedule was and use a template with a schedule one day previous
        # to it.
        sched = self.browse(cr, uid, my_id, context=context)

        # XXX - Work-around for companies that don't have days off for the first
        #       week of the contract. If you don't want to trigger this don't
        #       create a schedule called "No Rest".
        #
        dContract = False
        for contract in sched.employee_id.contract_ids:
            if not dContract or contract.date_start < dContract:
                dContract = contract.date_start
        if dContract:
            dSched = datetime.strptime(sched.date_start, OE_DFORMAT).date()
            dContract = datetime.strptime(dContract, OE_DFORMAT).date()
            while dContract.weekday() != 0:
                dContract += timedelta(days= -1)
            if dContract == dSched:
                tpl_obj = self.pool.get('hr.schedule.template')
                sched_tpl_ids = tpl_obj.search(cr, uid,
                                               [('name', 'ilike', '%no%rest%')],
                                               context=context)
                if len(sched_tpl_ids) > 0:
                    sched_tpl_id = sched_tpl_ids[0]
                    self.create_details(cr, uid, my_id,
                                        template_id=sched_tpl_id,
                                        context=context)
                    return my_id

        if sched.template_id.series_id:
            
            tmpl_id = sched.template_id.id
            
            # Find previous schedule:
            sched_ids = self.search(cr, uid, [('employee_id', '=', sched.employee_id.id),
                                              ('date_start', '<', sched.date_start)],
                                    order="date_start desc", limit=1, context=context)
            if len(sched_ids) > 0:
                prev_sched = self.browse(cr, uid, sched_ids[0], context=context)
                dPrevEnd = datetime.strptime(prev_sched.date_end, OE_DFORMAT).date()
                dCurrStart = datetime.strptime(sched.date_start, OE_DFORMAT).date()
                
                # If the previous schedule was also a rolling rest day schedule continue it.
                # If it's not the immediately preceding week, create all the intervening
                # schedules, but only if it was within the last 3 months.
                #
                if prev_sched.template_id and prev_sched.template_id.series_id \
                  and prev_sched.used_template_id and (dPrevEnd >= (dCurrStart + timedelta(days= -98))):
                    if dPrevEnd != (dCurrStart + timedelta(days= -1)):
                        dNextStart = dPrevEnd + timedelta(days= +1)
                        dNextEnd = dNextStart + timedelta(days= +6)
                        while dNextStart < dCurrStart:
                            _new_sched = {
                                'name': sched.employee_id.name +': '+ dNextStart.strftime(OE_DFORMAT) +' Wk '+ str(dNextStart.isocalendar()[1]),
                            ...
                            prev_id = self.create(cr, uid, _new_sched, context=context)
                            dNextStart += timedelta(days= +7)
                            dNextEnd = dNextStart + timedelta(days= +6)
                        prev_sched = self.browse(cr, uid, prev_id, context=context)
                    
                    tmpl_obj = self.pool.get('hr.schedule.template')
                    tmpl_ids = tmpl_obj.search(cr, uid,
                                               [('series_id', '=', prev_sched.used_template_id.series_id and prev_sched.used_template_id.series_id.id or False),
                                                ('sequence', '>', prev_sched.used_template_id.sequence)],
            ...
            self.create_details(cr, uid, my_id, template_id=tmpl_id, context=context)
        else:
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        detail_ids = []
        for schedule in self.browse(cr, uid, ids, context=context):
            # Do not remove schedules that have been verified
            if not self.deletable(cr, uid, schedule.id, context):
                continue
            
            detail_ids += [detail.id for detail in schedule.detail_ids]
            schedule_ids.append(schedule.id)
            
        # Delete the schedule details associated with the schedules
        #
        if len(detail_ids) > 0:
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        for sched in self.browse(cr, uid, ids, context=context):
            if sched.validated:
                raise osv.except_osv(_('Write Error'),
                                     _('You may not modify a validated schedule: %s') %(sched.name))
        
        return super(hr_schedule, self).write(cr, uid, ids, vals, context=context)
    
    def _validation(self, cr, uid, ids, unvalidate=False, context=None):
        ...
        detail_obj = self.pool.get('hr.schedule.detail')
        for sched in self.browse(cr, uid, ids, context=context):
            detail_ids = [detail.id for detail in sched.detail_ids]
            if len(detail_ids) > 0:
                if unvalidate:
                    detail_obj.unvalidate(cr, uid, detail_ids, context=context)
                else:
                    detail_obj.validate(cr, uid, detail_ids, context=context)
        
        self.write(cr, uid, ids, {'validated': action}, context=context)
        return True
    
    def validate(self, cr, uid, ids, context=None):
        
        return self._validation(cr, uid, ids, context=context)
    
    def unvalidate(self, cr, uid, ids, context=None):
        
        return self._validation(cr, uid, ids, unvalidate=True, context=context)
    
    def create_attendance_from_schedule(self, cr, uid, employee_id, day_str, context=None):
        ...
        det_ids = det_obj.search(cr, uid, [('schedule_id.employee_id', '=', employee_id),
                                           ('day', '=', day_str)],
        ...
        for detail in det_obj.browse(cr, uid, det_ids, context=context):
            ...
            lv_ids = lv_obj.search(cr, uid, [('employee_id', '=', employee_id),
                                             ('date_from', '<=', detail.date_end),
                                             ('date_to', '>=', detail.date_start)],
                                   context=context)
            
            # Adjust sign-in for ending leaves
            punch_in = detail.date_start
            for lv in lv_obj.browse(cr, uid, lv_ids, context=context):
                if (punch_in == detail.date_start and lv.date_to >= detail.date_start and lv.date_to < detail.date_end) \
                  or (lv.date_to >= detail.date_start and lv.date_to < detail.date_end and lv.date_to < punch_in):
                    dt = datetime.strptime(lv.date_to, OE_DTFORMAT) + timedelta(seconds= +1)
                    punch_in = dt.strftime(OE_DTFORMAT)
            ...
            # Adjust sign-out for starting leaves
            punch_out = detail.date_end
            for lv in lv_obj.browse(cr, uid, lv_ids, context=context):
                if (lv.date_from <= detail.date_end and punch_out == detail.date_end) \
                  or (lv.date_from <= detail.date_end and lv.date_from > punch_out):
                    dt = datetime.strptime(lv.date_from, OE_DTFORMAT) + timedelta(seconds= -1)
                    punch_out = dt.strftime(OE_DTFORMAT)
...
class schedule_detail(osv.osv):
    ...
    _columns = {
        ...
        'schedule_id': fields.many2one('hr.schedule', 'Schedule', required=True, ondelete='cascade'),
    ...
    def deletable(self, cr, uid, detail_id, context=None):
        ...
        detail = self.browse(cr, uid, detail_id, context=context)
        if detail.validated:
            return False
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        detail_ids = []
        for detail in self.browse(cr, uid, ids, context=context):
            if self.deletable(cr, uid, detail.id, context=context) or (context and context.get('force_delete', False)):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        for det in self.browse(cr, uid, ids, context=context):
            if det.validated:
                raise osv.except_osv(_('Write Error'),
                                     _('You may not modify a validated schedule detail: %s') %(det.name))
    ...
    def unvalidate(self, cr, uid, ids, context=None):
        ...
        self.write(cr, uid, ids, {'validated': False}, context=context)
        return True
...
class hr_schedule_template_series(osv.Model):
    
    _name = 'hr.schedule.template.series'
    _description = 'Employee Working Schedule Template Series'
...
class hr_schedule_template(osv.osv):
    ...
    _columns = {
        ...
        'series_id': fields.many2one('hr.schedule.template.series', 'Series'),
        'sequence': fields.integer('Series Sequence'),
        'tz': fields.selection(_tz_list, 'Time Zone', required=True),
    }
    ...
    def name_get(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            series_name = record.series_id and (record.series_id.name +' - ') or ''
            res.append((record.id, series_name + record.name))
        
        return res
    
    def onchange_series(self, cr, uid, ids, series_id, context=None):
        
        res = {'value': {'tz': False}}
        if series_id:
            series = self.pool['hr.schedule.template.series'].browse(cr, uid, series_id, context=context)
            res['value']['tz'] = series.tz
        return res
...
class hr_schedule_working_times(osv.osv):
    ...
    def name_get(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            dayofweek = '?'
            for _rec in DAYOFWEEK_SELECTION:
                if _rec[0] == record.dayofweek:
                    dayofweek = _rec[1] 
            name = record.template_id.name +' '+ dayofweek +' '+ record.hour_from +' to '+ record.hour_to
...
class hr_contract(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        while dTmp < dEnd:
            ...
            #
            sched_obj = self.pool.get('hr.schedule')
            prev_sched_ids = sched_obj.search(cr, uid, [('employee_id', '=', ee.id),
                                                        ('date_start', '<=', dTmp.strftime('%Y-%m-%d')),
                                                        ('date_end', '>=', dTmp.strftime('%Y-%m-%d'))],
                                              context=context)
            if len(prev_sched_ids) == 0:
                sched = {
                    'name': ee.name +': '+ dTmp.strftime('%Y-%m-%d') +' Wk '+ str(dTmp.isocalendar()[1]),
                    'employee_id': ee.id,
                    'template_id': ee.contract_id.schedule_template_id.id,
                    'date_start': dTmp.strftime('%Y-%m-%d'),
                    'date_end': (dTmp + timedelta(days= +6)).strftime('%Y-%m-%d'),
                }
                sched_obj.create(cr, uid, sched, context=context)
    ...
    def holidays_validate(self, cr, uid, ids, context=None):
        ...
        for lv in self.browse(cr, uid, ids, context=context):
            if lv.type == 'remove' and lv.holiday_type == 'employee':
                att_ids = att_obj.search(cr, uid, [('employee_id', '=', lv.employee_id.id),
                                                   ('name', '>=', lv.date_from),
                                                   ('name', '<=', lv.date_to)],
                                         order='name',
                                         context=context)
            ...
            self.adjust_schedule_by_leave(cr, uid, ids, context=context)
            
            if len(att_ids) > 0:
                
                # Remove attendance records. Make sure to remove attendance records for
                # the whole day (not just the part of it that fits in the leave). This
                # ensures that we don't have any dangling (open) attendances.
                att_days = []
                for att in att_obj.browse(cr, uid, att_ids, context=context):
                    if att.day not in att_days:
                        att_days.append(att.day)
                att2_ids = att_obj.search(cr, uid, [('employee_id', '=', lv.employee_id.id),
                                                    ('day', 'in', att_days)],
                                          context=context)
                att_ids += [i2 for i2 in att2_ids if i2 not in att_ids]
                att_obj.unlink(cr, uid, att_ids, context=context)
                
                # Re-create attendance around leave
                sched_obj = self.pool.get('hr.schedule')
                for day_str in att_days:
                    sched_obj.create_attendance_from_schedule(cr, uid, lv.employee_id.id, day_str,
                                                              context=context)
        return res
    
    def adjust_schedule_by_leave(self, cr, uid, ids, context=None):
        ...
        for leave in self.browse(cr, uid, ids, context=context):
            ...
            for detail in det_obj.browse(cr, uid, det_ids, context=context):
                ...
                # Partial day on first day of leave
                    ...
                    if leave.date_from == detail.date_end and detail.id not in unlink_ids:
                            unlink_ids.append(detail.id)
                    else:
