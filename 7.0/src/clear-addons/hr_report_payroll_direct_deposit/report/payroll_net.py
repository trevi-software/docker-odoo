...
class Parser(report_sxw.rml_parse):
    ...
    def get_payslip_runs(self, runs):
        ...
        for r in runs:
            ...
            for slip in r.slip_ids:
                ...
                # Do not include employee who do not use Direct Deposit or
                # whose effective date is after this pay slip.
                #
                if not slip.employee_id.use_direct_deposit:
                    continue
                elif (not dd) or (dd and dd.effective_date > slip.date_to):
                    continue
                else:
                    has_entries = True
                    break
    ...
    def get_details_by_payslip(self, payslips):
        ...
        for slip in payslips:
            ...
            # Do not include employee who do not use Direct Deposit or
            # whose effective date is after this payslip
            #
            if not slip.employee_id.use_direct_deposit:
                continue
            elif (not dd) or (dd and dd.effective_date > slip.date_to):
            ...
            if tmp['net'] > 0:
                tmp['bank_name'] = dd.bank_name
                tmp['account_name'] = dd.account_name
                tmp['net'] = self.fmt_float(tmp['net'], 2)
                res.append(tmp)
        return res
