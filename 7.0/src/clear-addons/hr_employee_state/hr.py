...
class hr_employee(osv.Model):
    ...
    def condition_start_onboarding(self, cr, uid, ids, context=None):
        
        res = False
        employee = self.browse(cr, uid, ids[0], context=context)
        
        if employee.contract_id and employee.contract_id.state not in ['draft']:
            res = True
    ...
    def state_active(self, cr, uid, ids, context=None):
        ...
        data = self.read(cr, uid, ids, ['status', 'saved_department_id'], context=context)
        for d in data:
    ...
    def state_inactive(self, cr, uid, ids, context=None):
        ...
        for d in data:
            ...
            if d['status'] and d['status'] == 'pending_inactive':
                vals.update({'department_id': d['saved_department_id'] and d['saved_department_id'][0] or False,
                            'saved_department_id': False})
        
            self.pool.get('hr.employee').write(cr, uid, d['id'], vals, context=context)
    ...
    def signal_reactivate(self, cr, uid, ids, context=None):
        ...
        for employee in self.browse(cr, uid, ids, context=context):
            ...
            netsvc.LocalService('workflow').trg_validate(uid, 'hr.employee', employee.id, 'signal_reactivate', cr)
...
class hr_contract(osv.Model):
    ...
    def re_activate(self, cr, uid, ids, context=None):
        ...
        for contract in self.browse(cr, uid, ids, context=context):
            ...
            if len(term_ids) == 0:
                ...
                # Trigger a status change of the employee and his contract(s)
                wkf.trg_validate(uid, 'hr.employee', contract.employee_id.id, 'signal_active', cr)
                if contract.state == 'pending_done':
                    wkf.trg_validate(uid, 'hr.contract', contract.id, 'signal_open', cr)
            else:
                
                for term in term_obj.browse(cr, uid, term_ids, context=context):
                    
                    # Trigger a status change of the employee and his contract(s)
                    if term.state == 'confirm':
                        wkf.trg_validate(uid, 'hr.employee.termination', term.id, 'signal_cancel', cr)
                    else:
                        term_obj.unlink(cr, uid, term.id, context=context)
    ...
    def state_common(self, cr, uid, ids, context=None):
        ...
        for contract in self.browse(cr, uid, ids, context=context):
            if contract.employee_id.status == 'new':
                wkf.trg_validate(uid, 'hr.employee', contract.employee_id.id, 'signal_confirm', cr)
    
    def state_trial(self, cr, uid, ids, context=None):
        """Override 'trial' contract state to also change employee state: new -> onboarding"""
        
        res = super(hr_contract, self).state_trial(cr, uid, ids, context=context)
        self.state_common(cr, uid, ids, context=context)
        return res
    
    def state_open(self, cr, uid, ids, context=None):
        """Override 'open' contract state to also change employee state: new -> onboarding"""
        
        res = super(hr_contract, self).state_open(cr, uid, ids, context=context)
        self.state_common(cr, uid, ids, context=context)
        return res
    ...
    def setup_pending_done(self, cr, uid, contract, term_vals, context=None):
        ...
        #
        open_contract = False
        ee = self.pool.get('hr.employee').browse(cr, uid, contract.employee_id.id, context=context)
        ...
        if open_contract or contract.state == 'done':
            if open_contract and contract.state != 'done':
                wkf.trg_validate(uid, 'hr.contract', contract.id, 'signal_done', cr)
            return 
        ...
        con_obj.write(cr, uid, contract.id, {'date_end_effective': term_vals['name']},
                      context=context)
        wkf.trg_validate(uid, 'hr.contract', contract.id, 'signal_pending_done', cr)
        
        # Set employee state to pending deactivation
        wkf.trg_validate(uid, 'hr.employee', contract.employee_id.id, 'signal_pending_inactive', cr)
...
class hr_job(osv.Model):
    ...
    def _get_job_position(self, cr, uid, ids, context=None):
        ...
        for ee in employees:
            if ee.job_id: res.append(ee.job_id.id)
        return res
    
    def _get_ids_from_contracts(self, cr, uid, ids, context=None):
        res = []
        contracts = self.pool.get('hr.contract').browse(cr, uid, ids, context=context)
        for c in contracts:
            if c.job_id: res.append(c.job_id.id)
            if c.end_job_id: res.append(c.end_job_id.id)
    ...
    _columns = {
        ...
        'no_of_employee': fields.function(_no_of_employee, string="Current Number of Employees",
            ...
            store = {
                ...
                'hr.contract': (_get_ids_from_contracts, ['job_id', 'end_job_id'], 10)
            },
