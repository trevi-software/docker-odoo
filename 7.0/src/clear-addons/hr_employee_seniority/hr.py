...
class hr_employee(osv.Model):
    ...
    def get_months_service_to_date(self, cr, uid, ids, dToday=None, context=None):
        ...
        if dToday == None:
            dToday = date.today()
        elif isinstance(dToday, dt.datetime):
            dToday = dToday.date()
        ...
        for ee in self.pool.get('hr.employee').browse(cr, uid, ids, context=context):
            ...
            res[ee.id] = (round(float((delta.years * 12) + delta.months) + date_part, 2), dInitial)
    ...
    def get_initial_employment_date(self, cr, uid, ids, context=None):
        ...
        for ee in self.browse(cr, uid, ids, context=context):
            contracts = self._get_contracts_list(ee)
            if len(contracts) > 0:
                res[ee.id] = datetime.strptime(contracts[0].date_start, OE_DATEFORMAT).date()
