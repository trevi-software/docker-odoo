...
class hr_transfer(osv.Model):
    ...
    def transfer_contract(self, cr, uid, contract, job_id, xfer_id, effective_date, context=None):
        ...
        if effective_date == contract.date_start:
            ...
            contract_obj.write(cr, uid, contract_id, new_vals, context=context)

            # Link to the new contract
            self.pool.get('hr.department.transfer').write(
                cr, uid, xfer_id, {'dst_contract_id': contract_id},
                context=context)
            return
        ...
        if c_id:
            ...
            dEnd = datetime.strptime(effective_date, DEFAULT_SERVER_DATE_FORMAT).date() + relativedelta(days= -1)
    ...
    def state_done(self, cr, uid, ids, context=None):
        ...
        for xfer in self.browse(cr, uid, ids, context=context):
            if datetime.strptime(xfer.date, DEFAULT_SERVER_DATE_FORMAT).date() <= today:
                self._check_state(cr, uid, xfer.src_contract_id.id, xfer.date, context=context)
                employee_obj.write(cr, uid, xfer.employee_id.id, {'department_id': xfer.dst_department_id.id},
                                   context=context)
                self.transfer_contract(cr, uid, xfer.src_contract_id, xfer.dst_id.id,
    ...
    def signal_confirm(self, cr, uid, ids, context=None):
        ...
        for xfer in self.browse(cr, uid, ids, context=context):
            ...
            if access_obj.check_groups(cr, uid, 'hr_transfer.group_hr_transfer'):
                wkf.trg_validate(uid, 'hr.department.transfer', xfer.id, 'signal_pending', cr)
            else:
                wkf.trg_validate(uid, 'hr.department.transfer', xfer.id, 'signal_confirm', cr)
    ...
    def try_pending_department_transfers(self, cr, uid, context=None):
        ...
        wkf = netsvc.LocalService('workflow')
        [wkf.trg_validate(uid, 'hr.department.transfer', xfer.id, 'signal_done', cr) for xfer in self.browse(cr, uid, xfer_ids, context=context)]
