...
class Parser(report_sxw.rml_parse):
    ...
    def get_employee_ids(self, department_id, seen_ids):
        ...
        src_contract_ids = []
        t_ids = transfer_obj.search(self.cr, self.uid, [('date', '>=', self.start_date),
                                                        ('date', '<=', self.end_date),
                                                        ('state', '=', 'done')])
        tdata = transfer_obj.read(self.cr, self.uid, t_ids, ['employee_id', 'src_contract_id'])
        for td in tdata:
            src_contract_ids.append(td['src_contract_id'][0])
        ...
        ee_ids = [data['employee_id'][0] for data in cdata if ((data['employee_id'][0] not in ee_ids) and (data['employee_id'][0] not in seen_ids) and (data['id'] not in src_contract_ids))]
