...
class payroll_period_end_1(osv.osv_memory):
    ...
    _defaults = {
        ...
        'closed': _get_closed,
        'ps_amendments_conf': False,
        'ps_amendments_draft': False,
    ...
    def create_payroll_register(self, cr, uid, ids, context=None):
        ...
        p_data = period_obj.read(cr, uid, period_id,
                                 ['company_id', 'name', 'date_start', 'date_end', 'schedule_id', 'register_id', 'state'],
                                 context=context)
        
        if p_data['state'] in ['payment', 'closed']:
            raise osv.except_osv(_('Invalid Action'), _('You cannot modify a payroll register once it has been marked for payment'))
        ...
        previous_register_id = p_data['register_id'] and p_data['register_id'][0] or False
        self.create_payslip_runs(cr, uid, period_id, previous_register_id, register_values, department_ids, s_data['contract_ids'],
    ...
    def create_payslip_runs(self, cr, uid, period_id, previous_register_id, register_values, dept_ids, contract_ids,
        ...
        if previous_register_id:
            self._remove_register(cr, uid, previous_register_id, context)
        
        # Create Payroll Register
        register_id = register_obj.create(cr, uid, register_values, context=context)
        ...
        for dept in dept_obj.browse(cr, uid, dept_ids, context=context):
            ...
            for ee in ee_obj.browse(cr, uid, ee_ids, context=context):
                ...
                if not pp_obj.process_employee(cr, uid, period_id,
                                               ee.id, context=context):
                    continue
        ...
        pp_obj.write(cr, uid, period_id, {'register_id': register_id}, context=context)
