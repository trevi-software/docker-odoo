...
class hr_contract(osv.Model):
    ...
    def _get_trial_date_end(self, cr, uid, context=None):
        ...
        return self._get_trial_date_end_from_start(cr, uid, datetime.now().date(), context=context)
    
    def _get_trial_date_end_from_start(self, cr, uid, dToday, context=None):
        
        res = False
        init = self.get_latest_initial_values(cr, uid, context=context)
        if dToday and init != None and init.trial_period and init.trial_period > 0:
    ...
    def get_latest_initial_values(self, cr, uid, today_str=None, context=None):
        ...
        res = None
        ids = init_obj.search(cr, uid, [('date', '<=', today_str), ('state', '=', 'approve')],
                              context=context)
