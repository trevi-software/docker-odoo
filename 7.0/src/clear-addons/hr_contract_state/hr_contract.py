...
class hr_contract(osv.osv):
    ...
    def close_previous_ending_contract(self, cr, uid, ids, context=None):

        wkf = netsvc.LocalService('workflow')
        for contract in self.browse(cr, uid, ids, context=context):
            for c in contract.employee_id.contract_ids:
                if c.date_end and c.date_end < contract.date_start and c.state == 'contract_ending':
                    wkf.trg_validate(uid, 'hr.contract', c.id, 'signal_done', cr)
        return

    def state_trial(self, cr, uid, ids, context=None):

        self.write(cr, uid, ids, {'state': 'trial'}, context=context)
        self.close_previous_ending_contract(cr, uid, ids, context=context)
        return True
    ...
    def state_open(self, cr, uid, ids, context=None):
        ...
        for i in ids:
            ...
            self.close_previous_ending_contract(cr, uid, [i], context=context)
    ...
    def state_done(self, cr, uid, ids, context=None):
        ...
        for i in ids:
            ...
            dToday = datetime.now().date()
            dEnd = dToday
            if not data.get('date_end', False):
