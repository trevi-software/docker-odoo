...
class hr_contract(osv.Model):
    ...
    def _remove_tags(self, cr, uid, employee_id, job_id, context=None):
        ...
        eedata = ee_obj.read(cr, uid, employee_id, ['category_ids'], context=context)
        job = self.pool.get('hr.job').browse(cr, uid, job_id, context=context)
        for tag in job.category_ids:
            if tag.id in eedata['category_ids']:
                ee_obj.write(cr, uid, employee_id, {'category_ids': [(3, tag.id)]}, context=context)
        return
    ...
    def _tag_employees(self, cr, uid, employee_id, job_id, context=None):    
        ...
        for tag in job.category_ids:
            if tag.id not in eedata['category_ids']:
                ee_obj.write(cr, uid, employee_id, {'category_ids': [(4, tag.id)]}, context=context)
        return
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        self._tag_employees(cr, uid, vals.get('employee_id', False), vals.get('job_id', False),
                            context)
            ...
            for data in prev_data:
                if data['id'] == contract.id:
                    if not vals.get('job_id', False) or (data.get('job_id', False) and data['job_id'][0] != vals['job_id']):
                        ...
                        self._remove_tags(cr, uid, contract.employee_id.id, prev_job_id,
                                          context=context)
                        if vals.get('job_id', False):
                            self._tag_employees(cr, uid, contract.employee_id.id, contract.job_id.id,
                                                context=context)
